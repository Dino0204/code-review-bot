...
import type { GitHubClient, PullRequestInfo, ReviewThread } from '../github/client'
import { REPLY_TOOL, buildReplyMessages, replyTools } from './prompt'
...
export async function answerThread(
  ...
  await github.replyToReviewComment(
    pr.number,
    thread.rootId,
    renderThreadReply(reply.reply, reply.suggestion ?? undefined, { model: llm.model }),
  )

  log.info(`#${pr.number} 쓰레드 응답 완료 (토큰 ${llm.totalUsage.total_tokens.toLocaleString()}, 모델 ${config.model})`)
...
async function requestReply(
  ...
  const messages = buildReplyMessages({ config, pr, thread, excerpt, instructions })
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    if (reply) return { reply: { ...reply, reply: reply.reply.trim() }, degraded: false }

    lastText = text
    log.warn(`모델이 도구를 호출하지 않았다 (${attempt}/2)`)
  }

  if (!lastText.trim()) throw new Error('모델이 답변도 도구 호출도 내놓지 않았다')
  return {
...
async function loadExcerpt(
  ...
  try {
    ...
    log.warn(`파일 발췌 실패(무시): ${thread.path} — ${error instanceof Error ? error.message : String(error)}`)
    return undefined
  }

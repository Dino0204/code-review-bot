...
import { FINDING_TOOL, READ_TOOL, SUMMARY_TOOL, buildReviewMessages, reviewTools } from './prompt'
...
async function gatherContext(deps: RunnerDeps, pr: PullRequestInfo): Promise<GatheredContext> {
  ...
  const sources = config.includeSources ? await loadSources(github, selected, pr.headSha, config) : []
  ...
  const loaded = await Promise.all(
    files.map(async (file) => {
      try {
        ...
        log.warn(`파일 원본 읽기 실패(무시): ${file.path} — ${error instanceof Error ? error.message : String(error)}`)
        return undefined
      }
    }),
  )

  const sources = loaded.filter((source): source is FileSource => source !== undefined)
  const partial = sources.filter((source) => source.partial).length
  log.info(`파일 원본 ${sources.length}/${files.length}개 로드` + (partial ? ` (${partial}개는 발췌)` : ''))
  return sources
}
...
export function chunkFiles(
  ...
  for (const file of files) {
    ...
    const rendered = renderFileDiff(file, config.maxFileChars).length + (sourceSizes?.get(file.path) ?? 0)
...
export async function runReview(deps: RunnerDeps, pr: PullRequestInfo): Promise<ReviewOutcome> {
  ...
  const sourceSizes = new Map(sources.map((source) => [source.path, sourceLength(source)]))
  const chunks = chunkFiles(context.diffFiles, config, context.instructions?.content.length ?? 0, sourceSizes)
...
async function requestReview(
  ...
  for (let round = 1; round <= maxRounds; round++) {
    const { toolCalls, text, raw } = await llm.chatWithTools(conversation, tools, options)
    ...
    if (reviewCalls.length > 0) {
      if (readCalls.length > 0) {
        log.warn(`리뷰 제출과 ${READ_TOOL} 을 함께 호출해 읽기 요청은 무시한다`)
      }
      return collectToolCalls(reviewCalls)
    }

    if (readCalls.length > 0) {
      const { message, granted } = await serveReads(github, readCalls, context, config, served, reads)
      reads += granted
...
async function serveReads(
  ...
  for (const call of calls) {
    ...
    if (used + granted >= config.maxExtraReads) {
      parts.push(`\`${path}\` — 읽기 상한(${config.maxExtraReads}개)에 도달해 읽지 않았다. 지금 있는 자료로 리뷰를 제출하라.`)
      continue
    }
    if (config.exclude.some((pattern) => minimatch(path, pattern, { dot: true }))) {
    ...
    try {
      ...
      log.warn(`${READ_TOOL} 실패: ${path} — ${error instanceof Error ? error.message : String(error)}`)
